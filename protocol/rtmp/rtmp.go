...
import (
	...
	"github.com/push-stream-service/utils/uid"

	"github.com/push-stream-service/av"
	"github.com/push-stream-service/configure"
	"github.com/push-stream-service/container/flv"
	"github.com/push-stream-service/protocol/rtmp/core"
...
func (s *Server) Serve(listener net.Listener) (err error) {
	...
	for {
		...
		fmt.Printf("new client, connect remote: ", conn.RemoteAddr().String(),
			"local:", conn.LocalAddr().String())
...
func (s *Server) handleConn(conn *core.Conn) error {
	...
	fmt.Println("start read message rtmp")
	if err := connServer.ReadMsg(); err != nil {
		conn.Close()
		log.Error("handleConn read msg err: ", err)
		fmt.Printf("handleConn read msg err: ", err)
		return err
	}

	appname, name, _ := connServer.GetInfo()

	fmt.Println("app appname %v, name %v", appname, name)
	...
	if connServer.IsPublisher() {
		...
		fmt.Println("start get channel name : %v", channel)
		...
		fmt.Println("GetStaticPushUrlList : ")

		reader := NewVirReader(connServer)

		fmt.Println("start HandleReader")
		...
		if s.getter != nil {
			...
			fmt.Printf("handleConn:writeType=%v", writeType)
			writer := s.getter.GetWriter(reader.Info())
			fmt.Println("start HandleWriter with getter != nil")
			s.handler.HandleWriter(writer)
		}
		if configure.Config.GetBool("flv_archive") {
			flvWriter := new(flv.FlvDvr)
			fmt.Println("start HandleWriter with flv_archive == true")
			s.handler.HandleWriter(flvWriter.GetWriter(reader.Info()))
		}
	} else {
		writer := NewVirWriter(connServer)
		log.Debugf("new player: %+v", writer.Info())
		fmt.Println("start HandleWriter with no publisher")
		s.handler.HandleWriter(writer)
	}
