...
func (h S3Handler) UploadFileFrRam(key string, file []byte) error {
	...
	_, err := s3.New(h.Session).PutObject(&s3.PutObjectInput{
		...
		Key:    aws.String(key),
		//ACL:                aws.String(S3_ACL),
		Body:               bytes.NewReader(file), // bytes.NewReader(buffer),
		ContentDisposition: aws.String("attachment"),
		// ContentLength:      aws.Int64(int64(len(buffer))),
		// ContentType:        aws.String(http.DetectContentType(buffer)),
		// ServerSideEncryption: aws.String("AES256"),
...
func NewS3Handle() S3Handler {
	sess, err := session.NewSession(&aws.Config{Region: aws.String(S3_REGION)})
	if err != nil {
		log.FatalF("session.NewSession - filename: %v, err: %v", err)
	}
